...
class Adam:
    class np:
        def __init__(self, lr: float, beta1: float, beta2: float):
            self.lr = lr
            self.beta1 = beta1
            self.beta2 = beta2
        ...
        def optimise(
            ...
            param = param - self.lr * m_hat / (np.sqrt(v_hat) + eps)
...
if __name__ == "__main__":
    ...
    opt_np = Adam.np(lr=0.1, beta1=0.9, beta2=0.999)

    param_pt = torch.tensor(init_param, requires_grad=True)
    opt_pt = torch.optim.Adam([param_pt], lr=0.1)
